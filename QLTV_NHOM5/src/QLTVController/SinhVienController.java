...
public class SinhVienController {
    ...
    public int kiemTraSV(String sv) {
        String sql = "select * from SinhVien where MaSinhVien = ? AND TrangThai = 1";
        // trạng thái 1 là bị tắt
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, sv);
            rs = pst.executeQuery();
            if (rs.next()) {
                // Sinh viên đã bị tắt trạng thái hoạt động
                return 0;
            }
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, ex.getMessage());
    ...
    public int kiemTraSinhVien_PhieuMuon(String sv) {
        String sql = "select * from PhieuMuon where MaSinhVien = ?";
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, sv);
            rs = pst.executeQuery();
    ...
    public int kiemTraSinhVien_PhieuMua(String sv) {
        String sql = "select * from PhieuMua where MaSinhVien = ?";
        try {
            pst = conn.prepareStatement(sql);
            pst.setString(1, sv);
            rs = pst.executeQuery();
